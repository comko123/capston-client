...
return (<div className="list_area" key={id}><h3 className="border_nickname">{title}</h3>
<div className="list"><Link className="list_link"to={`/detail/${index}/comment/${id}`}>
{content}</Link></div><div className="borderHR"/></div>)}})}</>)}
