...
const resultData =  await(await styleModify[0]({token:{
Authorization:`Bearer ${certifiedToken}`},info:state , settingUrl:`/my-clothes`})).data
...
<img src={pageback_btn} width={16} height={16} onClick={()=>navigate(-1)} alt="" className={"Myadress_pageback"}/>
...
catch (error) {console.log(error)}})()}} className={"Myadress_maincontainer_outer"}>
<div className={"Myadress_maincontainer"}>
{userIn.map((item,num)=>{return <UserSeasonClothing 
