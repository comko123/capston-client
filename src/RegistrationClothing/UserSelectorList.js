...
<input className={"Myadress_main_component_item_check"}type="checkbox"  value = {value} onClick={e=>{if(e.target.checked){add();dpc()}else{rmv()}}}/>
<span className={"Myadress_main_component_item_text"}style={includeElement[season]?.includes(clothing)?{color:"#4254ff"}:{color:"#54595E"}}>
{clothing}</span></div></>)}
