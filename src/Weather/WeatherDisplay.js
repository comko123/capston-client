...
export default function Display({humidity, temp, weather, dt}) {
    ...
    {dateValue.getDate() !== value.getDate() ? null :value.getHours() === 10
    || value.getHours() === 16 || value.getHours() === 20 || value.getHours() === 23 ?
    <div className={"NowWeather_card"}>
    <h1 className={"NowWeather_card_top"}>{value.getHours() === 10 ? 
    "아침" : value.getHours() === 16 ?"점심" : value.getHours() === 20 ?
    "저녁" : "밤"} </h1><div className={"NowWeather_card_maincontainer"}>
