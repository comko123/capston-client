...
{!isNaN(Math.round(data.main.temp)) && Math.round(data.main.temp)}</div></div>
<div className={"NowWeather_card_maincenter"}></div>
{data?.weather?.map((id,main)=>{return(
<div key={id} className={"NowWeather_card_mainright"}>
<h1 className={"NowWeather_card_title NowWeather_card_title_top"}>{main}</h1>
<img src={weather_cloudy} width={'51px'} height={'44px'} alt={main}/></div>)})}</div>
...
<div className={"NowWeather_card_title_value"}>{data.main.humidity+"%"}</div>
</div></div>}</>)}
