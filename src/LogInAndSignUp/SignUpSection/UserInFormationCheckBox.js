...
return (<><h3 className={`Sign_up_selectbox ${styleSelected&&title==="스타일"? "Sign_up_selectbox__active":null}`} 
...
{checkList.map((clothing, index) => {return (<SignUserCheckBox key={index} list={title} value = {clothing}/>)})}</div></>)}
